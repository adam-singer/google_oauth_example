...
Uri.fromString$ctor = function(uri) {
  Uri._fromMatch$ctor.call(this, const$0007.firstMatch(uri));
}
...
// ********** Code for top level **************
//  ********** Library google_oauth_example **************
// ********** Code for google_oauth_example **************
function google_oauth_example() {

}
google_oauth_example.prototype.run = function() {
  var LoginUri = "https://accounts.google.com/o/oauth2/auth?scope=https://www.googleapis.com/auth/userinfo.email+https://www.googleapis.com/auth/userinfo.profile&state=/profile&redirect_uri=http://financeCoding.github.com/google_oauth_example/google_oauth_example.html&response_type=token&client_id=669612457208-k2tqlu6f1ipv9c2kv71vt8m2ktlq3q8b.apps.googleusercontent.com";
  var a = _ElementFactoryProvider.Element$html$factory(("<a id='login' href='" + LoginUri + "'>Login</a>"));
  get$$document().body.get$elements().add(a);
  var queryArguments = new HashMapImplementation();
  var uri = new Uri.fromString$ctor(get$$window().location.href);
  print$(uri.fragment);
  uri.fragment.split$_("&").forEach((function (s) {
    var arg = s.split$_("=");
  ...
  var access_token = queryArguments.$index("access_token");
  if ($ne$(access_token)) {
    get$$document().query("#login").remove();
    var script = _ElementFactoryProvider.Element$tag$factory("script");
    script.set$src(("https://www.googleapis.com/oauth2/v1/userinfo?access_token=" + access_token + "&callback=callbackForJsonpApi"));
...
function dataReceived(e) {
  ...
  if ($ne$(data.$index("picture"))) {
    var h = _ElementFactoryProvider.Element$html$factory(("<h3>" + data.$index("name") + "</h3>"));
    get$$document().body.get$elements().add(h);
  }
  if ($ne$(data.$index("picture"))) {
    var a = _ElementFactoryProvider.Element$html$factory(("<a href='" + data.$index("link") + "'><img src='" + data.$index("picture") + "'></img></a>"));
    get$$document().body.get$elements().add(a);
  }
}
function main() {
  get$$window().get$on().get$message().add(dataReceived, false);
...
var const$0006 = Object.create(UnsupportedOperationException.prototype, {_message: {"value": "", writeable: false}});
var const$0007 = new JSSyntaxRegExp("^(?:([^:/?#.]+):)?(?://(?:([^/?#]*)@)?([\\w\\d\\-\\u0100-\\uffff.%]*)(?::([0-9]+))?)?([^?#]+)?(?:\\?([^#]*))?(?:#(.*))?$");
